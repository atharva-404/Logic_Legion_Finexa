...
const DEFAULT_HABITS: Habit[] = [
    { id: 'h1', title: 'Track Every Expense', description: 'Log all spending — awareness is step one.', icon: '📝', category: 'Tracking', streak: 0, points: 10, completed: false },
    { id: 'h2', title: 'Review Your Budget', description: 'Check if you are on track with the 50/30/20 rule.', icon: '📊', category: 'Planning', streak: 0, points: 15, completed: false },
    { id: 'h3', title: 'Save Before You Spend', description: 'Transfer to savings before discretionary spending.', icon: '🐖', category: 'Saving', streak: 0, points: 20, completed: false },
    { id: 'h4', title: 'No Impulse Buys', description: 'Wait 24 hours before any unplanned purchase over Rs 500.', icon: '🛑', category: 'Discipline', streak: 0, points: 15, completed: false },
    { id: 'h5', title: 'Learn Something New', description: 'Read one financial education article or card today.', icon: '📚', category: 'Learning', streak: 0, points: 10, completed: false },
    { id: 'h6', title: 'Check Financial Health', description: 'Review your health score and stress factors.', icon: '💚', category: 'Awareness', streak: 0, points: 10, completed: false },
];
...
export default function HabitChallenges() {
    ...
    function toggleHabit(id: string) {
        setHabits(hs => hs.map(h => h.id === id ? { ...h, completed: !h.completed, streak: !h.completed ? h.streak + 1 : Math.max(0, h.streak - 1) } : h));
    }

    useEffect(() => {
        GamificationAPI.getData().then((data: any) => {
            const apiBadges = data.badges || data.results || data;
            if (Array.isArray(apiBadges) && apiBadges.length > 0) {
                setBadges(apiBadges.map((b: any) => ({
                    id: String(b.id), name: b.name, description: b.description || '',
                    earned: b.earned ?? true, icon: b.icon || 'star', rarity: 'common',
                })));
            }
        }).catch(() => { });
    }, []);
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                ...
                {badges.length === 0 ? (
                    <div className="py-10 text-center">
                        ...
                        <p className="text-slate-400 text-sm">No badges unlocked yet.</p>
                        <p className="text-slate-500 text-xs mt-1">Keep completing habits to earn rewards!</p>
                    </div>
                ) : (
