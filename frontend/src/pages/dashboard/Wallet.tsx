...
import {
    ...
    RefreshCw, Check, Wifi, CreditCard as CardIcon, Loader2
} from 'lucide-react';
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                setBalance(parseFloat(w.balance) || 0);
                const raw = (d.results ?? d) as any[];
                if (raw.length) {
                    const mapped: Tx[] = raw.map((t: any) => ({
                        id: t.id,
                        type: t.type === 'ADD' || t.transaction_type === 'ADD' ? 'credit' : 'debit',
                        description: t.description || (t.type === 'ADD' ? 'Deposit' : 'Withdrawal'),
                        amount: parseFloat(t.amount) || 0,
                        date: t.timestamp || t.date || new Date().toISOString(),
                        category: t.type === 'ADD' || t.transaction_type === 'ADD' ? 'Transfer In' : 'Transfer Out',
                    }));
                    setTxList(mapped);
                }
            } catch { /* offline */ }
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center py-20">
                ...
                <span className="ml-3 text-slate-400">Loading wallet...</span>
            </div>
        );
    }
            ...
            <div className="flex items-center justify-between">
                ...
                <button onClick={async () => {
                    ...
                    try {
                        ...
                        setBalance(parseFloat(w.balance) || 0);
                        if (d.results?.length) {
                            const mapped: Tx[] = d.results.map((t: any) => ({
                                id: t.id,
                                type: t.type === 'ADD' || t.transaction_type === 'ADD' ? 'credit' as const : 'debit' as const,
                                description: t.description || (t.type === 'ADD' ? 'Deposit' : 'Withdrawal'),
                                amount: parseFloat(t.amount) || 0,
                                date: t.timestamp || t.date || new Date().toISOString(),
                                category: t.type === 'ADD' || t.transaction_type === 'ADD' ? 'Transfer In' : 'Transfer Out',
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                <div className="lg:col-span-3 space-y-4">
                    ...
                    <div className="card p-8 text-center space-y-3">
                        ...
                        <p className="text-xs text-3">Add a virtual card to get started</p>
                        <button onClick={() => setModal('add-card')} className="btn text-sm px-4 py-2 mx-auto">
